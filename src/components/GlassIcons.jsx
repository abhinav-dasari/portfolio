    ...
    <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ''}`}>
      {items.map((item, index) => (
        ...
        >
          ...
          <span className="absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-[12px] opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)] text-gray-800 dark:text-gray-200">
            {item.label}
          </span>
        </button>
      ))}
